...
After this package is installed, a user will be treated as "online" only if their most recent session activity occurred within a configurable time window (default: 30 minutes). This affects every place in CB that checks or displays online status. Kunena (the forum) already handles this independently via its own `showSessionType=2` / `showSessionStartTime=1800` settings and does not need patching.
...
- [ ] Milestone 1: System plugin `plg_system_cbuseronlinestatus` — project scaffolding, class autoloader for StatusField override
- [ ] Milestone 2: StatusField override class with configurable timeout
- [ ] Milestone 3: MessageTable override for PMS notification fix
- [ ] Milestone 4: Custom module `mod_cbuseronlinestatus` — scaffolding and manifest
- [ ] Milestone 5: Module helper with session queries (modes 1, 6, 7, 9)
...
CB's original code checks for online status in two patterns:
...
- `StatusField::getField()` in `components/com_comprofiler/plugin/user/plug_cbcore/library/Field/StatusField.php` — the online/offline indicator on profiles and user lists.
- `MessageTable::store()` in `components/com_comprofiler/plugin/user/plug_pms_mypmspro/library/Table/MessageTable.php` line 133 — suppresses email notifications if the recipient is "online".

Pattern B — direct SQL: The module `modules/mod_comprofileronline/mod_comprofileronline.php` queries `#__session` directly in four modes:

- Mode 1 (default "Online Users", line 394): `SELECT DISTINCT userid FROM #__session WHERE client_id = 0 AND guest = 0 ORDER BY time DESC`
- Mode 9 ("Online Connections", line 119): Same pattern with a JOIN to `#__comprofiler_members`.
- Mode 7 ("User Census", line 185): `SELECT COUNT(*) FROM #__session WHERE client_id = 0 AND guest = 0` (the `$onlineUsers` count).
- Mode 6 ("Online Statistics", line 248): Same count query plus offline and guest counts.

None of these queries filter by session age.
...
```text
    ...
    │   │   └── Field/
    │   │       └── StatusField.php          (override class)
    │   ├── language/
    │   │   └── en-GB/
...
`plg_system_cbuseronlinestatus/src/Extension/CbUserOnlineStatus.php` — Main plugin class in namespace `YakShaver\Plugin\System\Cbuseronlinestatus\Extension`. Extends `CMSPlugin`, implements `SubscriberInterface`, uses `DatabaseAwareTrait`. Subscribes to `onAfterInitialise`. In the handler, if this is a site client request, calls `spl_autoload_register` with a prepended autoloader method. The autoloader checks if the requested class is `CB\Plugin\Core\Field\StatusField` and, if so, loads the override from the plugin's own `src/Field/StatusField.php`. The timeout value is read from `$this->params->get('online_timeout', 1800)` and stored in a static property accessible by the override class.
...
After this milestone, the PMS (Private Messaging System) notification logic correctly treats users with stale sessions as offline, allowing email notifications to be sent.

This milestone requires the same autoloader approach. The autoloader in `CbUserOnlineStatus.php` is extended to also intercept `CB\Plugin\PMS\Table\MessageTable` (or whatever the correct FQCN is for the PMS MessageTable class).

First, determine the exact FQCN of the MessageTable class. The file `components/com_comprofiler/plugin/user/plug_pms_mypmspro/library/Table/MessageTable.php` must be read to confirm its namespace declaration. Based on the file path and CB conventions, it is likely `CB\Plugin\PMS\Table\MessageTable` or similar.

File to create:

`plg_system_cbuseronlinestatus/src/Table/MessageTable.php` — A copy of the original `MessageTable` class with the notification check at line 133 modified from:
...
However, since `MessageTable` is a much larger class (~400+ lines) with complex DB logic, duplicating the entire class is fragile. An alternative approach is to override only the `store()` method and call `parent::store()` — but this requires the override to extend the original, which creates a circular dependency with the autoloader.

A more practical approach: Instead of a full class override, check if the line can be isolated. If `MessageTable` is too large or complex, document this as a known limitation and recommend a minimal file patch for users who have `messages_notify_offline` enabled. Most CB installations do not enable this feature.

Before implementation, read the full `MessageTable.php` to assess its size and the feasibility of the override. If feasible (class is under ~200 lines with simple dependencies), proceed with the full override. If not, document it as a known limitation in the README and add a note about the manual patch needed.

Verification: If implemented, enable `messages_notify_offline` in CB PMS settings. Send a private message to a user whose session is >30 minutes old. The email notification should be sent (because the user is considered offline). Without the fix, the notification would be suppressed.
...
The session time filter clause used in all queries is:

```php
    AND (UNIX_TIMESTAMP() - s.time <= {$timeout})
```

where `{$timeout}` is the configured timeout in seconds. Use parameterized queries via Joomla's query builder where possible, but since CB's database API is used for compatibility with CB user rendering, the helper uses `$_CB_database` for consistency with the original module code.
...
```sql
    ...
    WHERE s.client_id = 0 AND s.guest = 0
    AND (UNIX_TIMESTAMP() - s.time <= ?)
    [AND s.userid NOT IN (?)]
    ORDER BY s.time DESC
```

`getOnlineStatistics(params, timeout, exclude)` — Mode 6. Returns an associative array with keys `onlineUsers`, `offlineUsers`, `guestUsers`. Three queries, each with the timeout filter on the session table.
...
Verification: Create a unit-level test by temporarily adding debug output in the helper. Or test via the module rendering in Milestone 6.
...
`mod_cbuseronlinestatus/tmpl/default.php` — Layout for modes 1 and 9 (Online Users and Online Connections). Receives `$cbUsers` (array of CBuser instances), `$preText`, `$postText`, `$params`, `$templateClass`. Renders an unordered list of users with CB profile links, identical structure to `modules/mod_comprofileronline/tmpl/default.php`. Includes CB plugin hook calls via `modCBOnlineHelper::getPlugins()` if available.

`mod_cbuseronlinestatus/tmpl/default_statistics.php` — Layout for mode 6 (Online Statistics). Receives `$onlineUsers`, `$offlineUsers`, `$guestUsers`, `$label`, `$separator`, `$params`. Renders counters, matching the structure of `modules/mod_comprofileronline/tmpl/_statistics.php`.

`mod_cbuseronlinestatus/tmpl/default_census.php` — Layout for mode 7 (User Census). Receives `$totalUsers`, `$latestUser`, `$onlineUsers`, `$usersToday`, `$usersWeek`, `$usersMonth`, `$usersYear`, `$label`, `$separator`, `$params`. Matches `modules/mod_comprofileronline/tmpl/_census.php`.
...
After this milestone, the package has been installed on the DDEV development site and all online-status indicators work correctly with the timeout filter.
...
6. Test each scenario:
   - Fresh login: user appears online in both the module list and their profile.
   - Stale session: manually set `time` to 31 minutes ago in `#__session` for a test user. Refresh pages — user should appear offline everywhere.
   - Statistics mode: change module mode to "Online Statistics". Counts should reflect only fresh sessions.
   - Census mode: change to "User Census". Online count should use the filtered query.
   - Online Connections mode: if the site uses CB connections, test mode 9.
   - PMS notifications (if MessageTable override implemented): enable `messages_notify_offline`, send a PM to a user with a stale session, verify email is sent.

Verification: All six scenarios pass. No PHP errors in Joomla's error log. No visual regressions on the frontend.
...
2. Refresh the site and verify all online-status indicators still work correctly (handled by the package now, not the file patches).

3. The `.original.php` backup files can optionally be deleted.

Verification: Site loads without errors. Online status indicators correctly show the 30-minute timeout. The CB files are now unmodified originals, safe for future CB updates.
...
```shell
    mkdir -p plg_system_cbuseronlinestatus/{services,src/{Extension,Field},language/en-GB}
...
1. The system plugin loads without errors and the `StatusField` override is active. Verified by checking that a user with a session row older than 30 minutes shows as "OFFLINE" on their profile page.

2. The custom module renders online users correctly in all four modes (Online Users, Online Connections, Online Statistics, User Census) with the timeout filter applied.

3. The two originally patched CB files (`mod_comprofileronline.php` and `StatusField.php`) have been reverted to their unmodified CB originals and the site continues to work correctly.

4. `make dist` produces three installable ZIPs without errors.

5. No PHP errors, warnings, or notices appear in Joomla's system log related to the package.
...
```php
    ...
    final class CbUserOnlineStatus extends CMSPlugin implements SubscriberInterface
    {
        ...
        // If $class === 'CB\\Plugin\\Core\\Field\\StatusField', requires override file.
        // If MessageTable override is implemented, also intercepts that FQCN.
    }
```
